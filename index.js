...
const answerQuestions = async (topic, topicName, showTimeUsed, showHint, retryIncorrectQuestions) => {
    console.log(`\n~ ${topicName} REVIEW ~\n`);
    let score = 0, response = null;
...
const freeplay = async () => {
    let cybersecurityScore = await answerQuestions(questions.cybersecurity, "CYBERSECURITY", true, false);
    console.log(`~ Cybersecurity Score: ${cybersecurityScore}/${questions.cybersecurity.length} ~`);
    let networkingScore =  await answerQuestions(questions.networking, "NETWORKING", true, false);
    console.log(`~ Networking Score: ${networkingScore}/${questions.networking.length} ~`);
    let programmingScore =  await answerQuestions(questions.programming, "PROGRAMMING", true, false);
...
const retryIncorrectQuestions = async () => {
    while (questionsToRetry.length != 0) {
        ...
        for (let i = 0; i < questionsToRetry.length; i++) {
            ...
            // If there is only one possible answer
            if (response.answer.toLowerCase() === questionsToRetry[i].answer.toLowerCase()) {
                console.log("CORRECT ✓");
                console.log();
                questionsToRetry.splice(i, 1);
                i--;
...
const training = async () => {
   // Ask user which topic 
    ...
    if (topic.input == "Cybersecurity") {
        let cybersecurityScore = await answerQuestions(questions.cybersecurity, "CYBERSECURITY", true, true, true);
        console.log(`~ Cybersecurity Score: ${cybersecurityScore}/${questions.cybersecurity.length} ~`);
    } else if (topic.input == "Networking") {
        let networkingScore =  await answerQuestions(questions.networking, "NETWORKING", true, true, true);
        console.log(`~ Networking Score: ${networkingScore}/${questions.networking.length} ~`);
    } else if (topic.input == "Programming") {
        let programmingScore =  await answerQuestions(questions.programming, "PROGRAMMING", true, true, true);
        console.log(`~ Programming Score: ${programmingScore}/${questions.programming.length} ~`);
    }
...
const main = async () => {
    ...
    const menuChoices = ["Freeplay (all topics)", "Training (specific topic)", "Review a topic"]
